...
class NLOS:
    ...
    def use_warp(self, mesh):
        from mdunet.warp_ops import ray_cast_warp, compute_ray_origins_and_directions
        ...
        d_shape = self.hidden.shape
        rays_d, rays_o = self.hidden.reshape(-1, 3), self.visible
        rays_o, rays_d = compute_ray_origins_and_directions(rays_o.to(self.device), rays_d.to(self.device))
        visibilty = 1-ray_cast_warp((vertices, faces), rays_o, rays_d).reshape(-1, d_shape[0], d_shape[1],).mean(-1)
        return visibilty
        
    
    def GetForwardModelFromMesh(self, mesh, batch_size=64, pinhole=False, return_vis=False, scenePoints=None, return_location=False):
        if "trimesh" in str(type(mesh)):
        ...
        occluder_scene = o3d.t.geometry.RaycastingScene()
        ...
        for i in range(0, len(scenePoints), batch_size):
            d, o = self.hidden[i:i+batch_size, :], self.visible
            d_shape = None
            if len(d.shape) > 2:
...
if __name__ == "__main__":
    ...
    M=(128, 128)        
    scene = PinsPeck(hidden_x_range=[0, 1.2192], hidden_y_range=(0.0, 0.0), hidden_z_range=[0., 1.2192],
                visible_x_range=[0.8, 1.803], visible_y_range=(2.3495, 1.0), visible_z_range=[0.0, 1.534],
