...
class Dataset(Dataset):
    def __init__(
        ...
        if not paths_global:
            ...
            if index is not None:
                ...
                self.paths = paths_global[overall*index : overall*index+overall][10:]
    ...
    def __getitem__(self, index):
        ...
        img2 = self.transform(im)


        return img, "/".join(path.split("/")[-3:]) #, img2



def torch_dataset(path="/data/fraji/ImageNet-Datasets-Downloader/imagenet/imagenet_images", 
                    batch_size=32,
                    image_size=128,
                    index=0):
...
def savez(f, parent_path, file):
    ...
    if not isExist:
        os.makedirs(parent_path)
        print("created = ",parent_path)
    filename = parent_path+file+".npz"

    np.savez(filename, measurement=f["measurement"], scene=f["scene"], pointcloud=f["pointcloud"], model_path=f["model_path"], box=f["box"], rotated=f["rotated"])
    # torch.save(p, filename)
    return True
...
def Grab_process(i, file):
    ...
    if os.path.isfile(f"{save_path}/imagenet/{folder}/{i+1:06d}_1.pt"):
        print(i, "done")
        return
    ...
    if boxes is None:
        boxes = np.array(model.generate_sweeping_bounding_boxes(0.2, 0.2, 0.2, 10))
        boxes = np.array(model.generate_sweeping_bounding_boxes(0.3, 0.3, 0.3, 5))
        boxes = np.array(model.generate_sweeping_bounding_boxes(0.4, 0.4, 0.4, 5))
        ...
        boxes = np.concatenate([boxes, model.generate_sweeping_bounding_boxes(0.25, 0.25, 0.25, 20)]) #np.array(model.PointDiv(ranges=40)) # np.concatenate([np.array(model.PointDiv(x_diff=0.3, y_diff=0.3, z_diff=0.3, ranges=20))])
    

    # try:

    val_ds = torch_dataset(path="/data/fraji/ImageNet-Datasets-Downloader/imagenet/imagenet_images", batch_size=repeated_m, index=i,
                                image_size=N[0])
    
    test_dl = DataLoader(val_ds, batch_size = repeated_m, pin_memory = True, num_workers = 2,)
    test_dl = cycle(test_dl)
    scene, image_path = next(test_dl)
    
    scene = scene.permute(0, 2, 3, 1)
    ...
    r = np.random.choice([0, 1, 2], p=[0.3, 0.35, 0.35])
    
    if r:
        r = 180 if r == 1 else np.random.choice(range(180))
        axis = np.random.choice(range(3))
        v = rotate_pointcloud(torch.from_numpy(mesh.vertices.astype(np.float32)), r, axis).numpy()
        mesh.vertices = point_as_occluder(v, box[:3], box[3:])


    mode = model.GetForwardModelFromMesh(mesh, 64, 0)
    m = model.get_measurement(scene, Model=mode).cpu()
    m=m/m.max()
    ...
    pc = PointCloud(coords=np.asarray(v), channels={})
    pc = pc.farthest_point_sample(2048)
    v = pc.coords.astype(np.float32)
    
    scene = (scene*255).type(torch.uint8)
    
    val = {"measurement":m.type(torch.float16), "scene":scene, "image_path":image_path, "model_path":file, "box":box, "rotated": r}
    val["pointcloud"] = torch.from_numpy(v)
    
    torchsave(val, save_path+"/imagenet/"+folder, f'/{i+1:06d}_1')
    
    # for j in range(len(m)):
        
    #     savez(val, save_path+"/imagenet/"+folder, f'/{i+1:06d}_{j}')
    # print("completed", i)
...
if __name__ ==  "__main__":
    ...
    parser.add_argument('--path_to_img', type=str, default="/data/fraji/ImageNet-Datasets-Downloader/imagenet/imagenet_images")
    parser.add_argument('--image_type', type=str, default="jpg")
    parser.add_argument('--save_path', type=str, default='/data/fraji')
    ...
    import random
    # ShapeNetCore.v2
    files = glob.glob("/data/fraji/ShapeNetCore.v2/**/**/models/**.obj") #[:10]#[13700:]#*4
    ...
    if path_to_img is not None:
        paths_global = glob.glob("/data/fraji/ImageNet-Datasets-Downloader/imagenet/imagenet_images/**/**.jpg") #[:10000]
    ...
    for i in range(len(files)):
        Grab_process(i, files[i])
    # Parallel(n_jobs=5, prefer="threads")(delayed(Grab_process)(i, file) for i, file in enumerate(files))
