...
Create a directory named `checkpoints` for the model weights:
```shell
mkdir checkpoints
```
Download the weights from [Google Drive](https://drive.google.com/drive/folders/1nsyxfwfQCCwNahumYxYH9Uu-Oa96P2X-?usp=sharing) and place them in the `checkpoints` folder.
...
Please note that the output of the demo is a dictionary mapping. Four distinct real-world/simulated shadow photographs are being used for reconstruction. **You can download these sample measurements directly from the original SSD repository here: [SSD Measurements](https://github.com/iscilab2020/Soft-shadow-diffusion/tree/main/measurements).**
...
If you have a new measurement, you may include it in a similar format. `.pt` measurements are the processed images `(1, 128, 128, 3)` saved as `torch.save(image, "m.pt")`.
...
```shell
CUDA_VISIBLE_DEVICES="0,1,2" python deepTrain.py --unet=1 --num_devices=3 --train_batch_size=128
```

Alternatively, you can utilize HuggingFace Accelerate for distributed data parallel scaling:
```shell
accelerate launch --multi_gpu --num_processes=3 Trainer.py --unet=1 --train_batch_size=64
```
