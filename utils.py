...
def awgn(s, SNR_min=100, SNR_max=None, L=1, return_snr=False):
    ...
    N0 = P/gamma
    n = torch.sqrt(N0).unsqueeze(1)*torch.rand(s.shape).to(device)
    s = s+n
    if return_snr:
...
def point_as_occluder(point0, box):
                      
    bounding_box_min, bounding_box_max = box[:3], box[3:]
       
       
    # Compute the bounding box of the mesh
    point0 = torch.from_numpy(point0) if isinstance(point0, np.ndarray) else point0
    ...
    ratio = mesh_dims/(mesh_dims[longest_dim_index]+1e-9)
    # ratio[ratio<0.5] = 0.5
...
def get_ratio(point0):
    ...
    return mesh_dims/(mesh_dims[longest_dim_index]+1e-9)


def generate_sweeping_bounding_boxes(min=-1, max=1, size=0.1, ranges=100, ratio=None):
    # Create ranges for x, y, and z
    
    occ_x_min=occ_y_min=occ_z_min = min
    occ_x_max=occ_y_max=occ_z_max = max
    x_range = torch.linspace(occ_x_min, occ_x_max, ranges)
    y_range = torch.linspace(occ_y_min, occ_y_max, ranges)
    z_range = torch.linspace(occ_z_min, occ_z_max, ranges)

    # Create a 3D grid of coordinates
    y, x, z = torch.meshgrid(y_range, x_range, z_range, indexing="ij")

    
    size_x = size*ratio[0] if ratio is not None else size
    size_y = size*ratio[1] if ratio is not None else size
    size_z = size*ratio[2] if ratio is not None else size

    # Adjust coordinates to ensure they are within bounds
    x = torch.clamp(x, min=occ_x_min, max=occ_x_max - size_x)
    y = torch.clamp(y, min=occ_y_min, max=occ_y_max - size_y)
    z = torch.clamp(z, min=occ_z_min, max=occ_z_max - size_y)

    # Create bounding boxes
    boxes = torch.stack((x, y, z, x + size_x, y + size_y, z + size_z), dim=-1)

    # Reshape to list of boxes
    boxes = boxes.reshape(-1, 6)

    return boxes


def generate_random_bounding_box(min=-1, max=1, size=0.1, ratio=None):
    occ_x_min =occ_y_min=occ_z_min = min
    occ_x_max =occ_y_max = occ_z_max = max
    ...
    # Generate random start coordinates within the specified ranges, accounting for diff
    x_start = torch.FloatTensor(1).uniform_(occ_x_min, occ_x_max - size_x)
    y_start = torch.FloatTensor(1).uniform_(occ_y_min, occ_y_max - size_y)
    z_start = torch.FloatTensor(1).uniform_(occ_z_min, occ_z_max - size_z)
    ...
    bounding_box = torch.cat([x_start, y_start, z_start, x_end, y_end, z_end])

    return bounding_box



def compute_sdf(points, centers, half_interval):
    """
    Compute the Signed Distance Function (SDF) for a point relative to a shape of box primitives.

    Args:
    - point (torch.Tensor): shape = [num_points, 3].
    - box_centers (torch.Tensor): shape = [num_boxes, 3].
    - box_half_size (torch.Tensor): Half the size of the box in each dimension (sx, sy, sz).

    Returns:
    - float: [num_points, 1] The SDF value for the point.
    """
    distance = torch.abs(points[None] - centers[:, None]) - half_interval #
    outside_dist = torch.relu(distance)
    inside_dist = torch.min(torch.max(distance, dim=-1).values, torch.tensor(0.0))
    return torch.min(torch.norm(outside_dist, dim=-1) + inside_dist, dim=0,  keepdim=True).values.t()


def compute_sdf(points, centers, half_interval, batch_size=4096):
    """
    Compute the Signed Distance Function (SDF) for points relative to a shape of box primitives in batches.

    Args:
    - points (torch.Tensor): shape = [num_points, 3].
    - centers (torch.Tensor): shape = [num_boxes, 3].
    - half_interval (torch.Tensor): Half the size of the box in each dimension (sx, sy, sz).
    - batch_size (int): The number of centers to process in each batch.

    Returns:
    - torch.Tensor: [num_points, 1] The SDF value for each point.
    """
    num_centers = centers.shape[0]
    sdf_values = torch.full((points.shape[0],), float('inf'), device=centers.device)

    for i in range(0, num_centers, batch_size):
        batch_centers = centers[i:i + batch_size]

        # Compute distances for the current batch
        distance = torch.abs(points[:, None] - batch_centers[None, :]) - half_interval
        outside_dist = torch.relu(distance)
        inside_dist = torch.min(torch.max(distance, dim=-1).values, torch.tensor(0.0))
        
        # Update SDF values with the minimum from this batch
        batch_sdf = torch.min(torch.norm(outside_dist, dim=-1) + inside_dist, dim=-1).values
        sdf_values = torch.min(sdf_values, batch_sdf)
...
class PointSDF(object):
    
    def __init__(self, bound = [-1, 1], resolution=256, save_points=True, device="cpu") -> None:
        ...
        self.resolution = resolution  # Grid size
        half_interval = 2.0 / self.resolution  / 2  # Half the size of a cell


        if self.save_points:
            x = torch.linspace(-1 + half_interval, 1 - half_interval, self.resolution )
            y = torch.linspace(-1 + half_interval, 1 - half_interval, self.resolution )
            z = torch.linspace(-1 + half_interval, 1 - half_interval, self.resolution )
            x, y, z = torch.meshgrid(x, y, z, indexing="ij")

            self.grid_points = torch.stack([x.ravel(), y.ravel(), z.ravel()], axis=-1, ).to(device)
        self.half_interval = half_interval
        
        
    def shift_point_into_grid(self, pointcloud_batch):
        # Constants  # Minimum occupancy value for x, y, z
        grid_size = self.resolution   # Size of the grid in each dimension
        cell_size = 2.0 / grid_size  # Size of each cell

        # Assuming pointcloud_batch is of shape (batch_size, num_points, 3)
        batch_size, num_points, _ = pointcloud_batch.shape

        # Normalize point cloud coordinates to the 0-127 range
        normalized_points = (pointcloud_batch - self.bound[0]) / cell_size

        # Convert to integer indices
        int_indices = normalized_points.long()

        # Filter out points that are outside the grid boundaries
        valid_indices_mask = (normalized_points >= 0) & (normalized_points < grid_size)
        valid_indices_mask = valid_indices_mask.all(dim=-1)  # Only keep points valid in all dimensions

        # Apply the mask to the indices
        valid_int_indices = [int_indices[b][valid_indices_mask[b]] for b in range(batch_size)]

        # Convert 3D indices to 1D indices for valid points
        grid_indices_1D = [idx[:, 0] + grid_size * (idx[:, 1] + grid_size * idx[:, 2]) for idx in valid_int_indices]

        return torch.unique(grid_indices_1D[0])
    ...
    def __getitem__(self, indexes):
        assert self.save_points, "Points are not Saved"
        return self.grid_points[indexes]
    
    def sample(self, batch_size, num_points):
        x = np.random.uniform(self.bound[0], self.bound[1], (batch_size,num_points))
        y = np.random.uniform(self.bound[0], self.bound[1], (batch_size,num_points))
        z = np.random.uniform(self.bound[0], self.bound[1], (batch_size,num_points))
        # Combine the sampled points
        sampled_points = np.stack([x, y, z], axis=-1)
      
        return torch.tensor(sampled_points, dtype=torch.float32).to(self.device)
    
    
    def compute_sdf(self, points, centers, shadow=True):
        sdf = compute_sdf(points, centers, self.half_interval)
    ...
    def generate_sweeping_bounding_boxes(self, size, ranges=100):
        return generate_sweeping_bounding_boxes(self.bound[0], self.bound[1], size=size, ranges=ranges)
    
    
    def generate_random_bounding_box(self, size):
        return generate_random_bounding_box(self.bound[0], self.bound[1], size=size)
...
class Query(object):
    ...
    def __init__(self, grid_size=128, side_length=1.02, device="cpu") -> None:
