...
class CombinedModel(nn.Module):
    def __init__(self, specs):
        ...
        if self.task in ('combined', 'modulation'):
            ...
            use_vae = specs["SdfModelSpecs"]["vae"]
            ...
            if use_vae:
                ...
                self.vae_model = AE(in_channels=feature_dim*3, latent_dim=modulation_dim, hidden_dims=hidden_dims)
    ...
    def train_modulation(self, x):
        ...
        if reduction == "none":
            sdf_loss = F.l1_loss(pred_sdf.squeeze(), torch.clamp(gt.squeeze(), -.5, .5), reduction=reduction)
            sdf_loss = reduce(sdf_loss, 'b ... -> b (...)', 'mean').mean()
            
        else:
            sdf_loss = F.l1_loss(gt.squeeze(), pred_sdf.squeeze(), reduction=reduction)

        loss = sdf_loss + min(1, epoch/100)*vae_loss
